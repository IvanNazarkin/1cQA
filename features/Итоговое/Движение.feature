...
Сценарий: Подготовка данных
	...
	И я проверяю или создаю для документа "РасходТовара" объекты:
		...
		| 'e1cib/data/Документ.РасходТовара?ref=8ad5a8a15929d47611efd56b2561bef3' | 'False'           | '1001'  | '18.01.2025 20:13:41' | 'True'     | 'e1cib/data/Справочник.Контрагенты?ref=8ca0000d8843cd1b11dc8d043d71007a' | 'e1cib/data/Справочник.Склады?ref=8ca1000d8843cd1b11dc8eb49faea67b' | ''       | 'e1cib/data/Справочник.ВидыЦен?ref=a9b000055d49b45e11db8c4c9d5c4226' | ''                    | 'e1cib/data/Справочник.Организации?ref=8d34000d8843cd1b11dd2bea12f94c65' |

	И я перезаполняю для объекта табличную часть "Товары":
		| 'Ссылка'                                                                | 'Товар'                                                             | 'Цена' | 'Количество' | 'Сумма' |
		| 'e1cib/data/Документ.РасходТовара?ref=8ad5a8a15929d47611efd56b2561bef3' | 'e1cib/data/Справочник.Товары?ref=8d3a000d8843cd1b11dd321ba7a30ab2' | 60000  | 1            | 60000   |
		| 'e1cib/data/Документ.РасходТовара?ref=8ad5a8a15929d47611efd56b2561bef3' | 'e1cib/data/Справочник.Товары?ref=8ca1000d8843cd1b11dc8eb360f70b4d' | 250    | 3            | 750     |


Сценарий: Движение РасчетОстатков
...
И в таблице 'Список' я перехожу к строке:
	...
	| '*'    | '1001'  | 'ООО "1000 мелочей"' | 'Магазин "Бытовая техника"' | 'Склад отдела продаж' | ''                      | 'Мелкооптовая' |
...
Тогда таблица 'Список' стала равной:
	...
	| '*'      | '*'           | '1'            | 'Магазин "Бытовая техника"' | '60 750,00' | ''       |
И В текущем окне я нажимаю кнопку командного интерфейса "Регистр продаж"
Тогда таблица 'Список' стала равной:
	| 'Период' | 'Регистратор' | 'Номер строки' | 'Покупатель'                | 'Товар'  | 'Количество' | 'Сумма'     |
	| '*'      | '*'           | '1'            | 'Магазин "Бытовая техника"' | 'Ремонт' | '1,00'       | '60 000,00' |
	| '*'      | '*'           | '2'            | 'Магазин "Бытовая техника"' | 'Торт '  | '3,00'       | '750,00'    |
...
Тогда табличный документ 'SpreadsheetDocument' равен:
	...
	| 'Сумма'         | '60 750 рублей (Шестьдесят тысяч семьсот пятьдесят рублей)' | ''           | ''       |
	| 'Вес'           | ''                                                          | ''           | ''       |
	| ''              | ''                                                          | ''           | ''       |
	| 'Товар'         | 'Цена'                                                      | 'Количество' | 'Сумма'  |
	| 'Ремонт'        | '60 000'                                                    | '1'          | '60 000' |
	| 'Торт '         | '250'                                                       | '3'          | '750'    |
